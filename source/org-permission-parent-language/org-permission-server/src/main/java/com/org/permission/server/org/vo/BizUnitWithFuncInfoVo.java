...
@ApiModel
@Data
public class BizUnitWithFuncInfoVo extends BaseDto implements Serializable {
	...
	@ApiModelProperty(value = "全局客户名字")
	private String custName;

	@ApiModelProperty(value = "内部客户名字")
	...
	public String getFunctionTypes() {
		if (!CollectionUtils.isEmpty(simpleFunctions)) {
			...
			simpleFunctions.forEach(item -> {
				FunctionTypeEnum functionTypeEnum = FunctionTypeEnum.getEnum(item);
				if (null != functionTypeEnum) {
					funcList.add(functionTypeEnum.getName());
				}
			});
			if (funcList.size() > 0) {
				return String.join(",", funcList);
			}
		}
		return null;
	}
